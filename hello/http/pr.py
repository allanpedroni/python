...
def send_email(message: EmailMessage, template_email, force_to_send=False):
    ...
    templateWithUrl = f"<a href={message.url}>PR {message.pr}</a> {template_email.replace(f'PR {message.pr}', '')}"

    answer = f'{first_name.capitalize()}, <br /><br />{templateWithUrl}<br /><br />Att'
